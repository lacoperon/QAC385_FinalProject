...
## Training using naive bayes (much less predictive)
# library(e1071)
# library(tm)
# n <- 6000
...
# one-hot encodes training dataset
X <- model.matrix(price ~ . -1, mdf_train)

model.xgboost <- xgboost(X, 
                         mdf_train$price,
                         objective="reg:linear",
                         nrounds = 100
                         )

yhat.xgboost <- predict(model.xgboost, model.matrix(price ~ . -1, mdf_test))
